...
def toC(module, max_degree=None):
    ...
    module.c_algebra = cMilnorAlgebra.construct(p=module.algebra.p, degree=max_degree+10)    
...
def indexToPyOpgen(module, degree, idx):
    ...
    b = cMilnorBasisElement.fromIndex(module.c_algebra, c_opgen.operation_degree, c_opgen.operation_index)
...
def act(module, op, element):
    ...
    for (op_basis_elt, c1) in op.items():
        op_idx = cMilnorBasisElement.toIndex(module.c_algebra, op_basis_elt)
        for ((elt_op, gen), c2) in element.items():
            elt_op_deg = elt_op.degree()
            elt_op = next(iter(elt_op))
            elt_op_idx = cMilnorBasisElement.toIndex(module.c_algebra, elt_op)
