...
uint MilnorAlgebra_getDimension(Algebra *public_algebra, uint degree) {
    MilnorAlgebraInternal *algebra = (MilnorAlgebraInternal *) public_algebra;
    assert(algebra->basis_table != NULL && degree < public_algebra->max_degree);
    return algebra->basis_table[degree].length;
}

MilnorBasisElement_list MilnorAlgebra_getBasis(MilnorAlgebra *public_algebra, uint degree){
    MilnorAlgebraInternal *algebra = (MilnorAlgebraInternal *) public_algebra;
...
void generateMilnorBasisPpartTable(MilnorAlgebraInternal *algebra, uint new_max_degree) {
    ...
    // array_print("profile_list: %s\n", profile_list, MAX_XI_TAU);
    ...
    for(uint current_degree = old_max_degree; current_degree < new_max_degree; current_degree++){
...
void MilnorAlgebra_freeBasis(MilnorAlgebra *public_algebra){
    ...
    khash_t(monomial_index_map) **name_table = algebra->basis_element_to_index_map;
    for(uint degree = 0; degree < algebra->public_algebra.algebra.max_degree; degree++) {
...
void GenerateMilnorBasis2(MilnorAlgebraInternal *algebra, uint old_max_degree, uint new_max_degree){
    ...
    for(uint degree = old_max_degree; degree < new_max_degree; degree++){
        ...
        for(uint i = 0; i < p_parts.length; i++){
            ...
            // Make a MBE with 0 q_part.
            MilnorBasisElement m = (MilnorBasisElement){0, 0, degree, x.length, x.p_part};
            // Put basis element into map. I think this has length at most 4 * 22 = 88 bytes.
            // 200 is a comfortable amount of extra space.
            char key[200];
            MilnorBasisElement_toKey(key, &m);
            int absent;
            khint_t bin = kh_put(monomial_index_map, name_table[degree], key, &absent);
            if(absent){
                // This strdup produces an obligation to free later.
                kh_key(name_table[degree], bin) = strdup(key);
            }
            kh_val(name_table[degree], bin) = current_degree_list.length;
            // Put basis element into list
...
void GenerateMilnorBasisGeneric(MilnorAlgebraInternal *algebra, uint old_max_degree, uint new_max_degree){
    ...
    generateMilnorBasisPpartTable(algebra, (new_max_degree + q - 1) / q);
...
MilnorBasisElement MilnorBasisElement_fromIndex(MilnorAlgebra *public_algebra, uint degree, uint idx) {
    MilnorAlgebraInternal *algebra = (MilnorAlgebraInternal*) public_algebra;
    return algebra->basis_table[degree].list[idx];
}

// Precondition: b is a valid MBE for the algebra -- it actually appears as a key in 
uint MilnorBasisElement_toIndex(MilnorAlgebra *public_algebra, MilnorBasisElement b){
    MilnorAlgebraInternal *algebra = (MilnorAlgebraInternal*) public_algebra;
    uint degree = b.q_degree + b.p_degree;
    kh_monomial_index_map_t *map = algebra->basis_element_to_index_map[degree];
    char key[200];
    MilnorBasisElement_toKey(key, &b);
    khint_t bin = kh_get(monomial_index_map, map, key);
    if(bin == kh_end(map)){
        MilnorBasisElement_toString(key, &b);
...
void MilnorProductEven(MilnorAlgebraInternal *algebra, uint *result, MilnorBasisElement r_elt, MilnorBasisElement s_elt){   
    ...
    do {
        ...
        if(coeff != 0) {
            ...
            uint idx = MilnorBasisElement_toIndex((MilnorAlgebra*)algebra, m);
...
void MilnorProductFullQpart(MilnorAlgebraInternal *algebra, uint *output, MilnorBasisElement m1, uint f) {
    ...
    uint idx = MilnorBasisElement_toIndex((MilnorAlgebra*)algebra, m1);
    ...
    for(uint k = 0; ( f & ~((1 << k) - 1) ) != 0; k++){
        ...
        for(uint idx = 0; idx < old_result_dimension; idx++){
            ...
            MilnorBasisElement mono = MilnorBasisElement_fromIndex((MilnorAlgebra*)algebra, old_result_degree, idx);
            for(uint i = 0; i < mono.p_length + 1; i++){
                ...
                uint out_idx = MilnorBasisElement_toIndex((MilnorAlgebra*)algebra, m);
...
void MilnorProductFull(MilnorAlgebraInternal *algebra, uint *result, MilnorBasisElement m1, MilnorBasisElement m2) {
    ...
    // Vector *m1_times_f_test = Vector_construct(p, m1_times_f_dimension, 0);
    // Vector_pack(m1_times_f_test, m1_times_f);
    // MilnorElement_print("m1_times_f: %s\n", algebra, m1_times_f_degree, m1_times_f_test);
    // Vector_free(m1_times_f_test);
    ...
    // Vector *result_test = Vector_construct(p, out_dimension, 0);
    for (uint i = 0; i < m1_times_f_dimension; i++) {
        if (m1_times_f[i] == 0) {
            continue;
        }
        uint coeff = m1_times_f[i];
        MilnorBasisElement er_mono = MilnorBasisElement_fromIndex((MilnorAlgebra *) algebra, m1_times_f_degree, i);
        ...
        for (uint j = 0; j < prod_dimension; j++) {
            ...
            // have nonzero Q part. The result of MilnorProductEven is guaranteed to have
            // 0's there. We could definitely make this slicker.
            if(prod[j] == 0){
                continue;
            }
            MilnorBasisElement m = MilnorBasisElement_fromIndex((MilnorAlgebra *) algebra, prod_degree, j);
...
void MilnorAlgebra_multiply(Algebra *public_algebra, Vector *result, uint coeff, uint r_degree, uint r_index, uint s_degree, uint s_index)  {
    MilnorAlgebraInternal *algebra = (MilnorAlgebraInternal*) public_algebra;
    MilnorBasisElement r, s;
    r = MilnorBasisElement_fromIndex((MilnorAlgebra*)algebra, r_degree, r_index);
    s = MilnorBasisElement_fromIndex((MilnorAlgebra*)algebra, s_degree, s_index);
...
//    n.p_part = (uint[1]){1};
    ...
    MilnorAlgebra *algebra = MilnorAlgebra_construct(3, true, NULL);
...
//    freeMilnorBasisPpartTable(algebra);
    ...
    //freeMilnorBasisQPartTable(algebra);
    MilnorAlgebra_generateBasis((Algebra*)algebra, 100);
...
//    }
    ...
    A = MilnorBasisElement_fromString(algebra, "P(4, 1)");
    B = MilnorBasisElement_fromString(algebra, "Q(0,1) * P(1)");
    uint A_idx = MilnorBasisElement_toIndex(algebra, A);
    uint B_idx = MilnorBasisElement_toIndex(algebra, B);
    uint A_deg = A.p_degree + A.q_degree;
    uint B_deg = B.p_degree + B.q_degree;

    uint output_degree = A_deg + B_deg;
    uint output_dimension = MilnorAlgebra_getDimension((Algebra*)algebra, output_degree);
    Vector *result = Vector_construct(algebra->p, output_dimension, 0);
    MilnorAlgebra_multiply((Algebra*)algebra, result, 1, A_deg, A_idx, B_deg, B_idx);
    string str1 = buffer;
    int len1 = MilnorElement_toString(str1, algebra, output_degree, result);
    string str2 = str1 + len1 + 1;
    int len2 = MilnorBasisElement_toString(str2, &A);
    string str3 = str2 + len2 + 1;
    MilnorBasisElement_toString(str3, &B);
    printf("%s * %s = %s\n", str2, str3, str1);
    free(A.p_part);
    free(B.p_part);
    Vector_free(result);
